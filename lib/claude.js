...
// Model that actually served the most recent API response (from the per-message `model` field).
// The API can silently substitute a fallback model (e.g. Fable 5 -> Opus 4.8 on a safety-classifier
// refusal, with ~1h sticky routing afterward), so this can differ from the configured init model.
let servedModel = null;

export function getInitInfo() {
  return { ...lastInitInfo, servedModel };
}

/** Called on /reset so /api/status doesn't report the previous session's model. */
export function resetInitInfo() {
  lastInitInfo = { model: null, apiKeySource: lastInitInfo.apiKeySource };
  servedModel = null;
}

// A dated snapshot id (claude-fable-5-20260601) still "matches" its alias (claude-fable-5).
...
let lastContextTokens = 0; // size of the prompt context on the most recent turn

/** Tokens in the context window as of the last turn (input + cache read + cache creation). */
export function getContextTokens() { return lastContextTokens; }

/** Called when the session is cleared/reset so the context gauge starts fresh. */
export function resetContextStats() { lastContextTokens = 0; }
  ...
  try {
    for await (const message of q) {
      ...
      if (message.type === 'assistant' && message.message?.content) {
        // The response's `model` field names the model that actually produced it. Surface
        // changes so a silent Fable->Opus fallback (refusal fallback / sticky routing) is visible.
        const respModel = message.message.model;
        if (respModel && respModel !== servedModel) {
          servedModel = respModel;
          const expected = lastInitInfo.model;
          const downgraded = !modelsMatch(respModel, expected);
          console.log(`[claude] Response served by ${respModel}${downgraded ? ` (configured: ${expected})` : ''}`);
          events.emit('model', { served: respModel, expected, downgraded });
        }
        // Context window = the prompt of THIS single API call (fresh input + cached prefix).
        // Capture it per assistant message; the last one of the turn is the true current window.
        // (Do NOT use the result message's usage — that is cumulative across every tool
        // round-trip in the turn and overcounts the window several-fold.)
        const au = message.message.usage;
        if (au) {
          lastContextTokens = (au.input_tokens || 0) +
      ...
      if (message.type === 'result') {
        ...
        // lastContextTokens was set from the per-call assistant usage above; emit it now.
        // (message.usage on the result is cumulative across the whole turn — not the window.)
        events.emit('usage', { contextTokens: lastContextTokens, ttftMs: message.ttft_ms });
