...
function buildSystemAppend(channel, systemPrompt) {
  ...
  return [timeStr, `Channel: ${channel}`, gatewayNote, systemPrompt, agentsMd].filter(Boolean).join('\n');
}
...
export async function sendMessage(content, { channel = 'web', planMode = false, model = null, systemPrompt = null } = {}) {
  ...
  if (!activeQuery) {
    ...
    try {
      startQuery(effectiveContent, { channel, model, planMode, systemPrompt });
    } catch (err) {
