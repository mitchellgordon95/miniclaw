...
export async function runClaude({
  ...
  const args = { prompt, channel, systemPrompt, sessionId, model, timeout, isolated, planMode, onDelta, onToolStart, onToolResult, onPlan, onStatus };

  // Isolated sessions run freely in parallel (no shared state)
  if (isolated) {
    return _runClaude(args);
  }

  // Main session is serialized via mutex
...
async function _runClaude({
  prompt, channel, systemPrompt, sessionId, model, timeout, isolated, planMode, onDelta, onToolStart, onToolResult, onPlan, onStatus,
}) {
  const config = loadConfig();
  ...
  try {
    const q = query({
      ...
      options: {
        ...
        mcpServers: {
          ...
          browser: {
            command: 'npx',
            args: ['@playwright/mcp', '--headless', '--user-data-dir', getHomePath('browser/openclaw/user-data')],
          },
        },
      },
    });
    ...
    for await (const message of q) {
      ...
      if (message.type === 'system' && message.subtype === 'status' && onStatus) {
