...
export async function runClaude({
  ...
  const args = { prompt, channel, systemPrompt, sessionId, model, timeout, planMode, onDelta, onToolStart, onToolResult, onPlan, onStatus, onAskUser };
...
async function _runClaude({
  prompt, channel, systemPrompt, sessionId, model, timeout, planMode, onDelta, onToolStart, onToolResult, onPlan, onStatus, onAskUser,
}) {
  const config = loadConfig();
          ...
          options: {
            ...
            canUseTool: async (toolName, input) => {
              if (toolName === 'AskUserQuestion' && onAskUser) {
                const answers = await onAskUser(input);
                return { behavior: 'allow', updatedInput: { questions: input.questions, answers } };
              }
              // Auto-allow everything else (bypassPermissions handles most, this catches interactive tools)
