import { writeFileSync, readFileSync, mkdirSync } from 'fs';
import { join } from 'path';
import { tmpdir, homedir } from 'os';
import twilio from 'twilio';
import { loadConfig } from './config.js';

const RESEND_SECRETS = join(homedir(), '.miniclaw/secrets/resend.json');

export function loadResendCreds() {
  try {
    return JSON.parse(readFileSync(RESEND_SECRETS, 'utf8'));
  } catch {
    return null;
  }
}

/**
 * Send an HTML email via Resend. Returns the message id or null.
 */
export async function sendEmailResend(to, subject, html) {
  const creds = loadResendCreds();
  if (!creds?.api_key) {
    console.error('[email] No Resend credentials configured');
    return null;
  }
  try {
    const payload = { from: creds.from, to: [to], subject, html };
    if (creds.reply_to) payload.reply_to = creds.reply_to;
    const res = await fetch('https://api.resend.com/emails', {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${creds.api_key}`,
        'Content-Type': 'application/json',
        'User-Agent': 'miniclaw/1.0',
      },
      body: JSON.stringify(payload),
    });
    if (!res.ok) {
      console.error(`[email] Resend send failed (${res.status}): ${await res.text()}`);
      return null;
    }
    const body = await res.json();
    return body.id || null;
  } catch (err) {
    console.error(`[email] Resend send error: ${err.message}`);
    return null;
  }
}

let _client = null;

function getTwilioClient() {
  if (_client) return _client;
  const config = loadConfig();
  _client = twilio(config.twilio.accountSid, config.twilio.authToken);
  return _client;
}

/**
 * Send a reply back to the originating channel.
 */
export async function sendReply(channel, content, meta = {}) {
  const route = meta.lastRoute || { channel: 'web' };

  switch (channel) {
    case 'web':
    case 'phone':
    case 'sms':
      // Reply to lastRoute (where the user last messaged from)
      if (route.channel === 'sms') {
        await sendSMS(route.from || loadConfig().twilio.allowFrom[0], content);
      }
      // Web and phone are already handled via WebSocket broadcast
      break;
    case 'email': {
      // Reply to an inbound email from Mitchell via Resend
      if (route.channel === 'email' && route.from) {
        const subject = route.subject
          ? (route.subject.startsWith('Re:') ? route.subject : `Re: ${route.subject}`)
          : 'Re: your message';
        const html = `<div style="font-family: sans-serif; white-space: pre-wrap;">${content}</div>`;
        await sendEmailResend(route.from, subject, html);
      }
      break;
    }
    case 'heartbeat':
      // Heartbeat just logs + broadcasts to web UI. Agent uses send-sms.sh if it wants to notify.
      break;
    case 'cron': {
      // Cron jobs use config-driven delivery
      const delivery = meta.delivery || 'none';
      const to = loadConfig().twilio.allowFrom[0];
      if (delivery === 'sms') {
        await sendSMS(to, content);
      } else if (delivery === 'sms-if-needed') {
        const skip = ['HEARTBEAT_OK', 'NOTHING_TO_REPORT'];
        if (!skip.includes(content.trim())) {
          await sendSMS(to, content);
        }
      }
      break;
    }
  }
}

/**
 * Send an SMS, splitting long messages at sentence boundaries.
 */
export async function sendSMS(to, content, { mediaUrl } = {}) {
  const config = loadConfig();
  const client = getTwilioClient();
  const chunks = splitMessage(content, 1600);

  for (let i = 0; i < chunks.length; i++) {
    const params = {
      body: chunks[i],
      from: config.twilio.phoneNumber,
      to,
    };
    // Attach media only to the first chunk
    if (i === 0 && mediaUrl) {
      params.mediaUrl = Array.isArray(mediaUrl) ? mediaUrl : [mediaUrl];
    }
    await client.messages.create(params);
  }
}

/**
 * Validate incoming Twilio webhook request.
 * Returns { valid, from, body } or { valid: false }.
 */
export function validateTwilioWebhook(req) {
  const config = loadConfig();

  // Validate Twilio signature
  const signature = req.headers['x-twilio-signature'] || '';
  const url = config.twilio.webhookUrl;
  const valid = twilio.validateRequest(
    config.twilio.authToken,
    signature,
    url,
    req.body || {}
  );

  if (!valid) return { valid: false, reason: 'invalid signature' };

  const from = req.body?.From;
  if (!config.twilio.allowFrom.includes(from)) {
    return { valid: false, reason: 'unauthorized number' };
  }

  // Extract MMS media attachments
  const numMedia = parseInt(req.body?.NumMedia || '0', 10);
  const media = [];
  for (let i = 0; i < numMedia; i++) {
    const mediaUrl = req.body?.[`MediaUrl${i}`];
    const contentType = req.body?.[`MediaContentType${i}`];
    if (mediaUrl) media.push({ url: mediaUrl, contentType });
  }

  return { valid: true, from, body: req.body?.Body || '', media };
}

/**
 * Download an image from Twilio and return as base64 + media type.
 */
export async function downloadTwilioImage(mediaUrl, contentType) {
  const config = loadConfig();
  const authHeader = 'Basic ' + Buffer.from(
    `${config.twilio.accountSid}:${config.twilio.authToken}`
  ).toString('base64');

  try {
    const res = await fetch(mediaUrl, {
      headers: { Authorization: authHeader },
      redirect: 'follow',
    });
    if (!res.ok) {
      console.error(`[sms] Failed to download image: ${res.status}`);
      return null;
    }
    const buffer = Buffer.from(await res.arrayBuffer());
    const actualType = res.headers.get('content-type') || contentType;
    // Normalize to Anthropic-accepted media types
    const mediaType = actualType?.includes('png') ? 'image/png'
      : actualType?.includes('gif') ? 'image/gif'
      : actualType?.includes('webp') ? 'image/webp'
      : 'image/jpeg';
    return { base64: buffer.toString('base64'), mediaType };
  } catch (err) {
    console.error(`[sms] Image download failed: ${err.message}`);
    return null;
  }
}

/**
 * Download media from Twilio and transcribe audio using OpenAI Whisper.
 * Returns the transcript text or null.
 */
export async function transcribeTwilioAudio(mediaUrl, contentType) {
  const config = loadConfig();
  const openaiKey = config.env?.OPENAI_API_KEY;
  if (!openaiKey) {
    console.error('[sms] No OPENAI_API_KEY configured for transcription');
    return null;
  }

  try {
    // Download audio from Twilio (requires Basic Auth)
    const authHeader = 'Basic ' + Buffer.from(
      `${config.twilio.accountSid}:${config.twilio.authToken}`
    ).toString('base64');

    const mediaRes = await fetch(mediaUrl, {
      headers: { Authorization: authHeader },
      redirect: 'follow',
    });
    if (!mediaRes.ok) {
      console.error(`[sms] Failed to download media: ${mediaRes.status}`);
      return null;
    }

    const audioBuffer = Buffer.from(await mediaRes.arrayBuffer());
    console.log(`[sms] Downloaded ${audioBuffer.length} bytes, content-type: ${contentType}`);
    const ext = contentType?.includes('mpeg') ? 'mp3'
      : contentType?.includes('ogg') ? 'ogg'
      : contentType?.includes('amr') ? 'amr'
      : contentType?.includes('mp3') ? 'mp3'
      : contentType?.includes('mp4') ? 'mp4'
      : contentType?.includes('wav') ? 'wav'
      : contentType?.includes('webm') ? 'webm'
      : 'mp3';

    const transcript = await transcribeAudioBuffer(audioBuffer, ext, contentType);
    console.log(`[sms] Transcribed audio: "${transcript}"`);
    return transcript;
  } catch (err) {
    console.error(`[sms] Transcription failed: ${err.message}`);
    return null;
  }
}

/**
 * Transcribe a raw audio buffer using OpenAI gpt-4o-transcribe.
 * Returns the transcript text or null. Shared by SMS voice notes and the web mic.
 */
export async function transcribeAudioBuffer(audioBuffer, ext = 'webm', contentType = 'audio/webm') {
  const config = loadConfig();
  const openaiKey = config.env?.OPENAI_API_KEY;
  if (!openaiKey) {
    console.error('[transcribe] No OPENAI_API_KEY configured');
    return null;
  }

  try {
    // Write to temp file then read back as File for reliable FormData upload
    const tmpPath = join(tmpdir(), `voice-${Date.now()}.${ext}`);
    writeFileSync(tmpPath, audioBuffer);
    const fileBuffer = readFileSync(tmpPath);

    const form = new FormData();
    const file = new File([fileBuffer], `voice.${ext}`, { type: contentType || 'audio/webm' });
    form.append('file', file);
    form.append('model', 'gpt-4o-transcribe');

    const whisperRes = await fetch('https://api.openai.com/v1/audio/transcriptions', {
      method: 'POST',
      headers: { Authorization: `Bearer ${openaiKey}` },
      body: form,
    });

    if (!whisperRes.ok) {
      const errText = await whisperRes.text();
      console.error(`[transcribe] API error (${whisperRes.status}): ${errText}`);
      return null;
    }

    const result = await whisperRes.json();
    return result.text?.trim() || null;
  } catch (err) {
    console.error(`[transcribe] failed: ${err.message}`);
    return null;
  }
}

/**
 * Synthesize speech from text using OpenAI gpt-4o-mini-tts.
 * Returns an MP3 Buffer or null. Used by the LP3 launcher's streaming TTS.
 */
export async function synthesizeSpeech(text, voice = 'alloy') {
  const config = loadConfig();
  const openaiKey = config.env?.OPENAI_API_KEY;
  if (!openaiKey) {
    console.error('[tts] No OPENAI_API_KEY configured');
    return null;
  }

  try {
    const res = await fetch('https://api.openai.com/v1/audio/speech', {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${openaiKey}`,
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        model: 'gpt-4o-mini-tts',
        voice,
        input: text,
        response_format: 'mp3',
      }),
    });

    if (!res.ok) {
      console.error(`[tts] API error (${res.status}): ${await res.text()}`);
      return null;
    }

    return Buffer.from(await res.arrayBuffer());
  } catch (err) {
    console.error(`[tts] failed: ${err.message}`);
    return null;
  }
}

function splitMessage(text, maxLen) {
  if (text.length <= maxLen) return [text];

  const chunks = [];
  let remaining = text;

  while (remaining.length > maxLen) {
    let splitAt = remaining.lastIndexOf('. ', maxLen);
    if (splitAt === -1 || splitAt < maxLen / 2) {
      splitAt = remaining.lastIndexOf(' ', maxLen);
    }
    if (splitAt === -1) {
      splitAt = maxLen;
    } else {
      splitAt += 1; // Include the space/period
    }
    chunks.push(remaining.slice(0, splitAt).trim());
    remaining = remaining.slice(splitAt).trim();
  }

  if (remaining) chunks.push(remaining);
  return chunks;
}
